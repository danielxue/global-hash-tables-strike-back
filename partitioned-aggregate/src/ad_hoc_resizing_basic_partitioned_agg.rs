// This is basically a carbon copy of basic_partitioned_agg but with some ad hoc capacity logic
// to make it work well with the unique key case of the resizing experiment. We validate that
// this causes a single resize on thread counts 1, 2, 4, 8, 16, and 32 but there is no
// further guarantee than that.
...
> PartitionedAggregator<K, V> for CountAgg<K, V, S> {
    ...
    fn into_finalizer(self) -> Box<dyn PartitionedAggregatorFinalizer<K, V, Agg=Self::Agg>> {
        ...
        let partition_capacity = self.capacity.div_ceil(self.threads);
...
> PartitionedAggregatorFinalizer<K, V> for CountAggFinalizer<K, V, S> {
    ...
    fn finalize_thread(&self) {
        ...
        let mut partition_tickets = HashTable::<(K, usize)>::with_capacity(self.capacity / 2);
