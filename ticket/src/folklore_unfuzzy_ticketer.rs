...
impl<K: Atomic, S> FolkloreUnfuzzyTicketer<K, S>
where
...
impl<K: Atomic + Send + Sync, S> Ticketer<K::Type> for FolkloreUnfuzzyTicketer<K, S>
where
    ...
    S: BuildHasher + Send + Sync + Default,
{
    ...
    fn with_capacity_and_threads(capacity: usize, threads: usize) -> Self {
        ...
        FolkloreUnfuzzyTicketer {
            ...
            ticketer: FuzzyCounter::new_with_step_size(1, 1),
            threads,
            bh: Default::default(),
        }
    }
